...
#       source_logit  =source_predict
       ...
       return source_predict,feature_source_f,feature_target_f,sim_matrix,sim_matrix_target,target_label_feature
...
#       acc=np.sum(label_smooth(test_predict)==test_labels)/len(test_predict)
   ...
   def predict(self,target):
       with torch.no_grad():
           self.eval()         
           feature_target_f=self.fea_extrator_f(target)
           test_logit=torch.matmul(torch.matmul(self.U,feature_target_f.T).T,self.stored_mat)
           test_cluster=torch.nn.functional.softmax(test_logit, dim=1)
           test_cluster=np.argmax(test_cluster.cpu().detach().numpy(),axis=1)
   ...
   def get_cos_similarity_by_threshold(self, cos_dist_matrix):
        ...
        sim_matrix = torch.where(cos_dist_matrix > self.threshold, similar,dissimilar)#大于域值的部分，会被选中吧
        return sim_matrix
