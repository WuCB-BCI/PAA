...
class Pairwise_Learning(nn.Module):
    def __init__(self,hidden_2,hidden_4,num_of_class,low_rank,max_iter,upper_threshold,lower_threshold,P):
        ...
        self.V = nn.parameter.Parameter(torch.randn(low_rank,hidden_4), requires_grad=True)
        self.U = nn.parameter.Parameter(torch.randn(low_rank,hidden_2), requires_grad=True)
        self.stored_mat = torch.matmul(self.V, P.T)

    def forward(self, target_feature, P):
        self.stored_mat = torch.matmul(self.V, P.T)
        target_predict = torch.matmul(torch.matmul(self.U, target_feature.T).T, self.stored_mat)
    ...
    def visualization(self, feature_target_f, target_labels, tsne=0):
    ...
    def predict(self, feature_target_f):
        with torch.no_grad():
            ...
            cluster_0_index, cluster_1_index, cluster_2_index = np.where(test_cluster == 0)[0], \
                                                                np.where(test_cluster == 1)[0], \
                                                                np.where(test_cluster == 2)[0]
    ...
    def get_parameters(self) -> List[Dict]:
        ...
        return params
class Domain_adversarial_model(nn.Module):
   def __init__(self,hidden_1,hidden_2,hidden_4,num_of_class,low_rank,max_iter):
       super(Domain_adversarial_model,self).__init__()
...
#       feature_source_g=feature_source_f
       ...
       self.P=torch.matmul(torch.inverse(torch.diag(source_label.sum(axis=0))+
                                         torch.eye(self.num_of_class).cuda()),torch.matmul(source_label.T,feature_source_g))


#       self.P=torch.matmul(torch.inverse(torch.diag(source_label.sum(axis=0))),torch.matmul(source_label.T,feature_source_g))
       self.stored_mat=torch.matmul(self.V,self.P.T)
       #self.stored_mat1 = torch.matmul(self.V1, self.P.T)
...
#       source_logit  =source_predict
       ...
       #return source_predict,feature_source_f,feature_target_f,sim_matrix_source,self.P
