...
from PAA_Basic_Architecture import Domain_adversarial_model, discriminator
from DataProcess import get_dataset_cross_session1, get_dataset_cross_allsession
from PAA_C import PAA_C
from PAA_L import PAA_L
...
def train_model(loader_train, loader_test, model, dann_loss, optimizer,paaC, paaL,
    ...
    for i in range(batch_num):
        ...
        paaL_loss = torch.tensor(0.).cuda()
        paaC_loss, selected_paaC, proto_dist = torch.tensor(0.).cuda(), 0, torch.tensor(0.).cuda()

        if parameter['paaC_or_paaL'] == 'paaC':
            if epoch < parameter['lda_add_epoch']:

                loss = cls_loss_s + transfer_loss + 0.01 * P_loss + boost_factor * cluster_loss_t
            else:

                paaC_loss, selected_paaC, proto_dist = paaC.get_loss(feat_s, feat_t, labels_s, pred_tar, model.P.detach())
                loss = cls_loss_s + transfer_loss + 0.01 * P_loss + boost_factor * cluster_loss_t + \
                       parameter['lda_weight'] * paaC_loss

        elif parameter['paaC_or_paaL'] == 'paaL':
            if epoch < parameter['lda_add_epoch']:
                loss = cls_loss_s + transfer_loss + 0.01 * P_loss + boost_factor * cluster_loss_t
            else:

                paaL_loss = paaL.get_loss(feat_s, feat_t, labels_s, pred_tar)
                loss = cls_loss_s + transfer_loss + 0.01 * P_loss + boost_factor * cluster_loss_t + \
                       parameter['lda_weight'] * paaL_loss
        ...
        paaC_loss_sum += paaC_loss.data
        paaL_loss_sum += paaL_loss.data
    ...
    writer.add_scalar('selected_for_paaC', selected_paaC, global_step=epoch)
    writer.add_scalar('paaC_loss_sum', paaC_loss_sum / batch_num, global_step=epoch)
    writer.add_scalar('paaL_loss_sum', paaL_loss_sum / batch_num, global_step=epoch)
    ...
    print('transfer_loss:', str(transfer_loss_sum / batch_num), '\np_loss:', str(P_loss_sum / batch_num),
          '\ncop_loss:', str(paaC_loss_sum / batch_num))
    ...
    if parameter['update_threshold']:
        ...
        if parameter['update_paaC']:
            paaC.update_PAAC_threshold(threshold)
    return record_dir
...
def train_and_test_GAN(parameter):  # pipeline
    ...
    #model
    model = Domain_adversarial_model(parameter['hidden_1'], parameter['hidden_2'],
    ...
    paaL = PAA_L(parameter['num_of_class'])
    paaC = PAA_C(num_layers=1, kernel_num=(5, 5), kernel_mul=(2, 2), num_classes=parameter['num_of_class'],
    ...
    for epoch in range(parameter['max_iter']):
        ...
        record_dir = train_model(loader_train, loader_test, model, dann_loss, optimizer,
                                 paaC, paaL,epoch, batch_num, record_dir, parameter)
