...
from PAA_M import Domain_adversarial_model, discriminator, Pairwise_Learning
...
def train_model(loader_train, loader_test,model, ada_classifity,rms_classifity, paaL, paaC,
    ...
    for i in range(batch_num):
        ...
        sim_matrix_target, pred_tar = ada_classifity(feature_target_f, model.P)
        ...
        paaL_loss = paaL.get_loss(feature_source_f, feature_target_f, labels_s, pred_tar)
        paaC_loss, _, _ = paaC.get_loss(feature_source_f, feature_target_f, labels_s, pred_tar, model.P.detach())

        cls_loss_sum += cls_loss.data  #
        transfer_loss_sum += transfer_loss.data

        loss = cls_loss + transfer_loss * 1 + paaL_loss * 0.5 + paaC_loss * 0.5 + 0.01 * P_loss + \
               boost_factor * cluster_loss * 0.1 + boost_factor * cluster_loss1 * 0.1
    ...
    for i in range(batch_num):
        ...
        bce_loss = -(torch.log(sim_matrix + eta) * estimated_sim_truth) - (1 - estimated_sim_truth) * torch.log(
            1 - sim_matrix + eta)
        cls_loss = torch.mean(bce_loss)
        loss_3 = transfer_loss + (cls_loss + cluster_loss + cluster_loss1) #Adv_loss + Ral_loss
...
def train_and_test_GAN(parameter):  ## pipeline for PR-PL model
    ...
    model = Domain_adversarial_model(hidden_1, hidden_2, hidden_4, num_of_class,low_rank, max_iter).cuda(0)
    ...
    for epoch in range(max_iter):
        ...
        cls_loss_sum, transfer_loss_sum = train_model(loader_train, loader_test,
                                                      model, ada_classifity,rms_classifity, paaL, paaC,
