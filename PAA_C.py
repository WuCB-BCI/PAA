...
class PAA_C(object):
    ...
    def cal_PAAC_loss(self, source, target, nums_S, nums_T):
        """
        Args:
            nums_S, nums_T: 每一个类别所包含的样本数 example:[300, 300, 300]
        """
        assert(len(nums_S) == len(nums_T)), \
        ...
        #print(inter)
        #print("")
        paaC = intra if inter is None else intra - inter
        return paaC
    ...
    #获得loss
    def get_loss(self, source, target, s_label, t_label, source_proto):#原型特征
        """
        Args:
            source,target:
        ...
        if len(nums_T) == 0:
            ...
            paaC = self.cal_PAAC_loss(source, target, nums_S, nums_T)

            # sim_matrix = torch.clamp(PAAC_loss, min=1e-7, max=1 - 1e-7)
            return paaC.cuda(), selected_num, self.proto_dist.cpu()
